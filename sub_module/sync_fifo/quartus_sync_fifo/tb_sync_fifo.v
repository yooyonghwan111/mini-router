module tb_sync_fifo ();
	...
	reg						wr_en;
	reg						rd_en;
	reg	[D_WIDTH-1:0]	tdata_in;
	
	wire	[D_WIDTH-1:0] 	tdata_out; 
	...
	initial begin
		...
		rst_n=0; wr_en=0; rd_en=0; tdata_in=0; 
		$display("[%0t]  rst_n=%b; wr_en=%b; rd_en=%b; tdata_in=%d", $time, rst_n, wr_en, rd_en, tdata_in); 
		...
		for (i=0; i<DEPTH; i=i+1) begin
			@(posedge clk) 
			wr_en = 1; tdata_in=$random; wdata[i]=tdata_in;
		...
		for (i=0; i<DEPTH; i=i+1) begin	
			...
			@(posedge clk) 
			$display("[%0t][READ] rd_en=%b, rd_val=%d", $time, rd_en, tdata_out);
			if(tdata_out == wdata[i]) $display("[%0t][WRITE/READ Value Matched !] wr_val=%d, rd_val=%d", $time, wdata[i], tdata_out);
			else $error("[%0t][WRITE/READ Value MisMatched !] wr_val=%d, rd_val=%d", $time, wdata[i], tdata_out);
		...
		$display("[%0t] 4-1. full overflow check (%0d write) ", $time, DEPTH+1);
		...
		for (i=0; i<DEPTH; i=i+1) begin
			...
			wr_en = 1; tdata_in=$random; wdata[i]=tdata_in;
			@(posedge clk)
			$display("[%0t][%0d][WRITE] wr_en=%b, wr_val=%d, wptr_before=%b, wptr=%b", $time, i+1, wr_en, tdata_in, wptr_before, u0.wptr);
		...
		@(posedge clk)
		$display("[%0t] 4-2. empty overflow check (%0d read) ", $time, DEPTH+1);

		for (i=0; i<DEPTH; i=i+1) begin
			@(posedge clk) 
			$display("[%0t][%0d][READ] rd_en=%b, rd_val=%d, rptr_before=%b, rptr=%b", $time, i+1, rd_en, tdata_out, rptr_before, u0.rptr);
		end
			rptr_before = u0.rptr;
			@(posedge clk) 
			$display("[%0t][%0d][READ] rd_en=%b, rd_val=%d, rptr_before=%b, rptr=%b", $time, i+1, rd_en, tdata_out, rptr_before, u0.rptr);
